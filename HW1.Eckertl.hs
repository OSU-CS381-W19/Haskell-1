...
maxInt :: Tree -> Int
maxInt (Leaf h) = h
maxInt (Node h t1 t2) = max (max h (maxInt t1)) (max h (maxInt t2))
...
minInt :: Tree -> Int
minInt (Leaf h) = h
minInt (Node h t1 t2) = min (min h (minInt t1)) (min h (minInt t2))
...
--sumInts _ = 0
sumInts (Leaf k) = k
sumInts (Node n t1 t2) = n + sumInts t1 + sumInts t2
...
preorder :: Tree -> [Int]
preorder (Leaf x) = [x]
preorder (Node h l r) = h : preorder l ++ preorder r
...
inorder :: Tree -> [Int]
inorder (Leaf x) = [x]
inorder (Node h l r) = inorder l ++ [h] ++ inorder r

--https://stackoverflow.com/questions/44634397/haskell-binary-search-tree
...
isBST :: Tree -> Bool
isBST (Leaf _) = True
isBST (Node h l r) = ((maxInt l) < h) && ((minInt r) > h) && isBST l && isBST r
